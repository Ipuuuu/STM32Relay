...
namespace Receiver
{
    ...
    {
        if (relayPin < RELAY_PIN_COUNT)
            return RELAY_PIN_MAP[relayPin];
    ...
    public:
        ...
        // NEW: Set current config version
        void setConfigVersion(uint8_t version) { configVersion = version; }
        // NEW: Get config version
        uint8_t getConfigVersion() const { return configVersion; }
        // NEW: Manually trigger reset flag (call in setup after reset)
        void markJustReset() { stateFlags |= STATE_FLAG_RESET; }
        // NEW: Clear reset flag (after master acknowledges)
        void clearResetFlag() { stateFlags &= ~STATE_FLAG_RESET; }
    ...
    private:
        ...
        // NEW: Handle extended commands
        void handleExtendedCommand();
    };
